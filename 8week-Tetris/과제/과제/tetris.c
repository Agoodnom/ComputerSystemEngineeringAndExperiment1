...
double timediff(struct timeval t0, struct timeval t1)
{
	return (double)((t1.tv_sec - t0.tv_sec) + (t1.tv_usec - t0.tv_usec) / 1000000.0);
}
...
void DrawChange_rec(char f[HEIGHT][WIDTH], int command, int currentBlock, int blockRotate, int blockY, int blockX) {
	int i, j;
	int preX, preY, preR;
	preX = blockX;
	preY = blockY;
...
void recommendedPlay() {
	...
	it_val.it_value.tv_sec = 70 / 1000;
	it_val.it_value.tv_usec = (70 * 1000) % 1000000;
...
int evalState(int lv, char f[HEIGHT][WIDTH], int r, int y, int x) {
	...
	addscore = AddBlockToField(f, nextBlock[lv], r, y, x);
	addscore += 10 * DeleteLine(f);
	addscore += 6 * y;
	addscore += 4 * avg;
	addscore += 6 * side_touched;
	addscore += -5 * hole;
	addscore += -9 * (maxy - miny);
	addscore += -4 * totalhole;


	return addscore;
}

long tree_size(RecNode *root) {
	int i;
	long size = sizeof(root);
	for (i = 0; i < CHILDREN_MAX; ++i) {
		if (root->c[i])
			size += tree_size(root->c[i]);
	}
	return size;
}
...
void Print_score_sec() {
	...
	t = timediff(tstart, time);
	recommendtime += timediff(rstart, rstop);
	recommendSpace += tree_size(recRoot);
	double spaceEfficiency = 0;
	...
	move(5, WIDTH + 25);
	printw("time(t)			= %10.2f   s", recommendtime);

	move(6, WIDTH + 25);
	if (recommendSpace < 1000) {
		printw("space(t)			= %10.2f   B", (double)recommendSpace);
	}
	else if (recommendSpace < 1e6) {
		printw("space(t)			= %10.2f  KB", (double)recommendSpace / 1000);
	}
	else if (recommendSpace < 1e9) {
		printw("space(t)			= %10.2f  MB", (double)recommendSpace / 1e6);
	}
	else if (recommendSpace < 1e12) {
		printw("space(t)			= %10.2f  GB", (double)recommendSpace / 1e9);
	}
	else if (recommendSpace < 1e15) {
		printw("space(t)			= %10.2f  TB", (double)recommendSpace / 1e12);
	}
	else {
		printw("space(t)			= %10.2f  PB", (double)recommendSpace / 1e15);
	}
	move(8, WIDTH + 25);
	printw("time efficiency		= %10.2f", score / recommendtime);

	move(9, WIDTH + 25);
	spaceEfficiency = (double)score / recommendSpace;
	if (spaceEfficiency >= 1) {
		printw("space efficiency		= %10.2f / B", (double)score / recommendSpace);
	}
	else if (spaceEfficiency >= 1e-3) {
		printw("space efficiency		= %10.2f /KB", (double)score / recommendSpace * 1e3);
	}
	else if (spaceEfficiency >= 1e-6) {
		printw("space efficiency		= %10.2f /MB", (double)score / recommendSpace * 1e6);
	}
	else if (spaceEfficiency >= 1e-9) {
		printw("space efficiency		= %10.2f /GB", (double)score / recommendSpace * 1e9);
	}
	else if (spaceEfficiency >= 1e-12) {
		printw("space efficiency		= %10.2f /TB", (double)score / recommendSpace * 1e12);
	}
	else {
		printw("space efficiency		= %10.2f /PB", (double)score / recommendSpace * 1e15);
	}
